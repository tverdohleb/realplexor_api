...
class Realplexor
{
    ...
    public function __construct($host, $port, $namespace = null, $identifier = "identifier")
    {
        ...
        if (version_compare(PHP_VERSION, "5.2.1", "<")) {
            throw new Realplexor\Exception("You should use PHP 5.2.1 and higher to run this library");
        }
    }
     ...
     * Throw Realplexor\Exception in case of error.
     *
    ...
    public function send($idsAndCursors, $data, $showOnlyForIds = null)
    {
        ...
        foreach ((array)$idsAndCursors as $id => $cursor) {
            ...
            if (!preg_match('/^\w+$/', $id)) {
                throw new Realplexor\Exception("Identifier must be alphanumeric, \"$id\" given");
            }
            $id = $this->_namespace . $id;
            if ($cursor !== null) {
                if (!is_numeric($cursor)) {
                    throw new Realplexor\Exception("Cursor must be numeric, \"$cursor\" given");
                }
                $pairs[] = "$cursor:$id";
            } else {
                $pairs[] = $id;
    ...
    public function cmdWatch($fromPos, $idPrefixes = null)
    {
        ...
        if (!preg_match('/^[\d.]+$/', $fromPos)) {
            throw new Realplexor\Exception("Position value must be numeric, \"$fromPos\" given");
        }
        // Add namespaces.
        if (strlen($this->_namespace)) {
            if (!$idPrefixes) $idPrefixes = array(""); // if no prefix passed, we still need namespace prefix
    ...
    private function _send($identifier, $body)
    {
        ...
        try {
            ...
            if (!$f) {
                throw new Realplexor\Exception("Error #$errno: $errstr");
            }
            if (@fwrite($f, $data) === false) {
                throw new Realplexor\Exception($php_errormsg);
            }
        ...
        if ($result) {
            ...
            if (!preg_match('{^HTTP/[\d.]+ \s+ ((\d+) [^\r\n]*)}six', $headers, $m)) {
                throw new Realplexor\Exception("Non-HTTP response received:\n" . $result);
            }
            if ($m[2] != 200) {
                throw new Realplexor\Exception("Request failed: " . $m[1] . "\n" . $body);
            }
            if (!preg_match('/^Content-Length: \s* (\d+)/mix', $headers, $m)) {
                throw new Realplexor\Exception("No Content-Length header in response headers:\n" . $headers);
            }
            $needLen = $m[1];
            $recvLen = $this->_strlen($body);
            if ($needLen != $recvLen) {
                throw new Realplexor\Exception("Response length ($recvLen) is different than specified in Content-Length header ($needLen): possibly broken response\n");
            }
            return $body;
        }
        return $result;
